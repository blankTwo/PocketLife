...
### 1.1 开发环境搭建
- [x] 初始化 Taro 4.x 项目（React + TypeScript）
- [x] 配置微信小程序云开发环境
- [x] 配置 Zustand 状态管理
- [ ] 配置 ESLint + Prettier 代码规范

### 1.2 项目结构规划
- [x] 创建目录结构（pages / components / store / services / utils）
- [x] 创建云函数目录结构
- [x] 定义 TypeScript 类型文件（Pet 实体等）

### 1.3 云数据库设计
- [ ] 创建 pets 集合（需在微信开发者工具中手动创建）
- [ ] 设置 openid 唯一索引（需在微信开发者工具中手动创建）
- [x] 定义字段结构及初始值
...
### 2.1 云函数 - 基础接口
- [x] getPet：获取/创建宠物（含时间结算 + 死亡检测 + 进化检测）
- [x] doAction：执行行为（喂食/玩耍/睡觉）
- [x] reborn：重生接口

### 2.2 核心工具函数
- [x] settle()：时间衰减结算
- [x] checkDeath()：死亡判定
- [x] checkEvolution()：进化判定
- [x] clamp()：数值边界处理

### 2.3 前端 - 主页面
- [x] 宠物状态展示（hunger / mood / energy 数值）
- [x] 三个行为按钮（喂食 / 玩耍 / 睡觉）
- [x] 按钮点击调用云函数
- [x] 状态更新后刷新 UI
- [x] Canvas 宠物动画（表情 + 动作动画）

### 2.4 前端 - 状态管理
- [x] Zustand store 设计（petStore）
- [x] 宠物数据获取与缓存
...
### 3.3 进化系统
- [x] 进化阶段展示（baby / adult / master）
- [ ] 进化时的 UI 反馈（可选：简单提示）

### 3.4 行为冷却
- [x] 全局 5s 冷却逻辑
- [ ] 按钮置灰 + 倒计时提示（目前只有置灰，无倒计时）
